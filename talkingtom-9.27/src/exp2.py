...
runnable_sequence = RunnableSequence(
    RunnableLambda(lambda _: run_chain("What's happening in technology?")),
    RunnableLambda(lambda _: run_chain("Wait, tell me a silly joke.")),
    RunnableLambda(lambda _: run_chain("What's happening in technology?"))
)

# Invoke the sequence
response = runnable_sequence.invoke(input="What's happening in technology?")
print(response)

#### Define the Intervention Function
def intervention_engine(task, intervention_task):
    # Step 1: Generate initial response
    print("Original Task: ", task)
    original_response = run_chain(task=task)
    print("Original Response: ", original_response)

    # Step 2: Perform the intervention task
    print("\n[Intervention] User: ", intervention_task)
    intervention_response = run_chain(task=intervention_task)
    print("Intervention Response: ", intervention_response)

    # Step 3: Resume the Original Task
    print("\n[Resume] Well, continuing from where we left off...")
    resume_prompt = f"{original_response.content.strip()} Let's continue. {task}"
    resumed_response = run_chain(task=resume_prompt)
